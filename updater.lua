...
function download_files(filelist,count)
 ...
 ws2812.write(string.char(brg,brg,0):rep(count-remaining)..string.char(0,0,brg):rep(remaining))
  ...
  tmr.alarm(0, 1000, 0, function()
   ...
   http.get(update_base_url..filename, nil, function(code,data)
    if (code ~= 200) then
      ...
      ws2812.write(string.char(brg,brg,0):rep(count-remaining)..string.char(brg,0,0)..string.char(0,0,brg):rep(remaining-1))
    else
     print("ok,size:",#data)
